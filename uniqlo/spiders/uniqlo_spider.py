...
class UniqloSpider(scrapy.Spider):
    ...
    def parse_category(self, response):
        ...
        for item in response.xpath('//ul[contains(@class, "products-grid")] \
                /li/a/@href').extract():
            yield scrapy.Request(item, callback=self.parse_item)

    # Helper function, makes a str out of a list
    ...
    def parse_item(self, response):
        ...
        image_list_urls = [i + '.jpg' for i in image_list_urls]

        title = response.xpath('//h1[@id="goodsNmArea"]/span/text()').extract()[0]
        description = response.xpath('//div[@id="prodDetail"]/div[contains(@class, "content")] \
                /text()').extract()[1].strip()
        code = response.xpath('//ul[@class="basic"]/li[@class="number"]/text()').extract()
        item_code  = code[0].split()[-1]
        price = response.xpath('//div[@class="price-box"]/p/span[@id="product-price-7"] \
                /text()').re('(\d+.\d+)')[0]
        ...
        for col in cols:
            ...
            image_path = hashlib.md5(image_urls).hexdigest()+'.jpg'
